...
function renderPinned() {
	...
	const postsToRender = isGithubContentEnabled()
		? pinned
		: pinned.filter((p) => p.source !== "yaml");
...
export function renderPage() {
	if (!entryGrid) return;
	// Filter out GitHub (YAML) posts if disabled
	const notesToRender = isGithubContentEnabled()
		? notes
		: notes.filter((n) => n.source !== "yaml");
...
export function initPosts() {
	...
	if (portfolioSaveButton) {
		portfolioSaveButton.addEventListener("click", async () => {
			...
			if (
				validatedTitle.length !== title.length ||
				validatedContent.length !== content.length
			) {
