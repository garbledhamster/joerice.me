...
/**
 * Interaction behavior (no visible "Edit" button):
 * - Single click: view post
 * - Admin Ctrl/⌘ + click: edit post (local/firestore only)
 * - Admin double-click: edit post (local/firestore only)
 * - Admin long-press on touch: edit post (local/firestore only)
 */
...
function loadLocalPosts() {
  ...
  try {
    ...
    parsed.forEach(post => {
      if (!post?.id || !post?.title || typeof post?.content !== "string")
        return;
      localPosts.push({
        id: post.id,
        title: post.title,
...
async function loadFirestorePosts() {
  ...
  try {
    ...
    const query =
      typeof orderedQuery.select === "function"
...
async function openEditorForEntryElement(el) {
  if (!el) return;
  if (!isAdminUser()) return;
  if (!ensureAdmin("edit portfolio post")) return;

  const source = el.dataset.source || "";
  const id = el.dataset.id || "";
  const url = el.dataset.url || "";

  // Preloaded (YAML) posts have no source/id and are not editable.
  if (!source || !id) return;
  if (!["local", "firestore"].includes(source)) return;

  if (source === "local") {
    const localPost = getLocalPostById(id);
    if (!localPost) {
      setPortfolioStatus("Unable to find that post.");
      return;
    }
    openPortfolioEditor({
      id: localPost.id,
  ...
  const postsRef = getPostsCollectionRef();
  if (!postsRef) {
    setPortfolioStatus("You must be logged in to edit Firestore posts.");
    return;
  }

  try {
    const docId = url.startsWith("firestore:")
      ? url.replace("firestore:", "")
      : id;
    const doc = await postsRef.doc(docId).get();
    if (!doc.exists) {
      setPortfolioStatus("Unable to find that post.");
      return;
    }
    ...
    openPortfolioEditor({
      id: doc.id,
      source: "firestore",
      title: data["Title"] || el.textContent || "Untitled",
      content: data["Body"] || "",
      createdDate: data["Created Date"] || null,
    });
  } catch (error) {
    console.warn("Unable to open editor for Firestore post.", error);
    setPortfolioStatus("Unable to edit this post right now.");
  }
}

function attachClickHandlers() {
  $$(".entry").forEach(el => {
    // Single click: view (delayed slightly to allow dblclick to cancel)
    el.addEventListener("click", event => {
      // Admin shortcut: Ctrl/⌘ click edits local/firestore posts (no visible edit button)
      if (isAdminUser() && (event.ctrlKey || event.metaKey)) {
        ...
        openEditorForEntryElement(el);
        return;
      }

      const existing = clickTimers.get(el);
    ...
    // Double click: admin edit for local/firestore posts (no visible edit button)
    ...
    // Long-press (touch): admin edit for local/firestore posts
    el.addEventListener("pointerdown", event => {
      if (!isAdminUser()) return;
      if (event.pointerType !== "touch") return;

      const existing = longPressTimers.get(el);
      if (existing) clearTimeout(existing);
    ...
    const clearLongPress = () => {
      const t = longPressTimers.get(el);
      if (t) {
        clearTimeout(t);
        longPressTimers.delete(el);
      }
...
function filterEntries() {
  $$(".entry").forEach(el => {
    ...
    const hide =
      selectedTags.size && ![...selectedTags].some(t => tags.includes(t));
...
function renderPinned() {
  ...
  pinnedGrid.innerHTML = pinned
    .map(
      p =>
        `<a class="entry" data-tags="${p.tags.join("|")}" data-url="${p.url}">${
          p.title
        }</a>`
    )
    .join("");
...
export function renderPage() {
  ...
  entryGrid.innerHTML = slice
    .map(n => {
      ...
      return `<a class="entry" data-tags="${(n.tags || []).join(
        "|"
      )}" data-url="${n.url}"${sourceAttr}${idAttr}>${n.title}</a>`;
    })
    .join("");
...
export async function openPost(url) {
  ...
  try {
    if (url.startsWith("local:")) {
      ...
      const storedContent =
        typeof window.getStoredPostContent === "function"
          ? window.getStoredPostContent(url)
          : null;
      const content = storedContent ?? (data.content || "");
      currentPost = { url, data, content, source: "preloaded" };
      renderPostContent(content);
    }
  } catch {
    currentPost = { url, data: null, content: "", source: "preloaded" };
    if (postContentEl) {
  ...
  if (portfolioDeleteButton) {
    portfolioDeleteButton.addEventListener("click", async () => {
      ...
      const index = localPosts.findIndex(
        post => post.id === editingLocalPostId
      );
      if (index === -1) {
        setEditorStatus("Unable to find that post.");
        return;
