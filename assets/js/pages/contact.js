...
async function handleSubmit(e) {
  ...
  try {
    ...
    if (window.grecaptcha?.execute) {
      const token = await window.grecaptcha.execute('6LcYpE4sAAAAALOW_7HWe81eUCnFkGFu-e8dAW_S', { action: 'submit' });
...
export function initContact() {
  ...
  if (closeBtn) {
    cleanupFns.push(addListener(closeBtn, 'click', () => closeModal('contactModal')));
  }

  // Modal backdrop click
  const modal = $('#contactModal');
  if (modal) {
    cleanupFns.push(addListener(modal, 'click', e => {
